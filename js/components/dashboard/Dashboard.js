...
(() => {
  ...
  const DashboardV2 = ({ appointments: initialAppointments = [], credentials: initialCredentials = [], setView, user, onOpenSettings }) => {
    ...
    const setup = React.useMemo(() => {
      try {
        if (typeof computeSetupChecklistReadOnly === "function") return computeSetupChecklistReadOnly({ user, appointments: data.appointments });
      } catch {}
    ...
    const steps = React.useMemo(() => {
      const list = [
        ...
        { key: "gps", title: "GPS Mileage", subtitle: "Start + stop a trip", done: !!setup?.gps?.done, cta: { label: "Open GPS", onClick: () => { setView?.("finances"); showToast("Go to Finances → GPS Mileage to start a trip.", "info"); } } },
        { key: "export", title: "Export Preview", subtitle: "Download CSV for taxes", done: !!setup?.export?.done, cta: { label: "Open Export", onClick: () => { setView?.("finances"); showToast("Go to Finances → Export to download a CSV.", "info"); } } },
      ];
    ...
    return (
      <div className="f5-dash">
        ...
        <div className="f5-grid-3" style={{ marginBottom: 14 }}>
          <div className="f5-card f5-kpi" onClick={() => setView?.("schedule")} role="button" tabIndex={0}>
          ...
          <div className="f5-card f5-kpi" onClick={() => setView?.("finances")} role="button" tabIndex={0}>
          ...
          <div className="f5-card f5-kpi" onClick={() => setView?.("credentials")} role="button" tabIndex={0}>
        ...
        <div className="f5-grid-2" style={{ marginBottom: 14 }}>
          ...
          <div className="f5-card">
            ...
            <div style={{ fontSize: 14, fontWeight: 800, color: "#475569", marginBottom: 12 }}>{upcoming.length} upcoming signings</div>
        ...
        <div className="f5-grid-main" style={{ marginBottom: 14 }}>
          <div className="f5-card">
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0,1fr))", gap: 14, marginBottom: 14 }}>
              <div>
                ...
                <span className="f5-delta f5-delta-up" style={{ marginTop: 8 }}><i className="fas fa-arrow-trend-up" /> 12.5% vs last month</span>
              </div>
            ...
            <RevenueChart series={revenue12m} />
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div className="f5-card">
              ...
              {loading ? (
                <div className="theme-text-muted" style={{ fontWeight: 800 }}><i className="fas fa-spinner fa-spin" /> Loading…</div>
              ) : upcoming.length ? (
                <>
                  {upcoming.map((u, idx) => (
            ...
            <div className="f5-card">
              ...
              <div className="f5-action-item"><div className="f5-action-dot f5-action-dot-ok" /><div className="f5-action-text">Invoices tracked</div></div>
              <div className="f5-action-item"><div className="f5-action-dot f5-action-dot-warn" /><div className="f5-action-text">{openRisks} expiring credential(s)</div></div>
            </div>
          </div>
        </div>
            ...
            {steps.list.map((s) => (
              <div key={s.key} className="f5-card" style={{ boxShadow: "none", padding: 14 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10 }}>
                  ...
                  {!s.done ? <button className="f5-btn-secondary" style={{ height: 36 }} onClick={s.cta.onClick}>{s.cta.label}</button> : null}
                </div>
              </div>
            ))}
          </div>
